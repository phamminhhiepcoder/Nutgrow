...
function handleFormQuizSubmitByIdAndMethod(formId, onSuccess, method = "post") {
  const form = $("#" + formId);
  if (form === null) {
    console.error("Form is null!");
  } else {
    ...
    const mcRatio = $("#mcRatio").val();
    const essayRatio = $("#essayRatio").val();

    if (mcRatio !== undefined) formData.append("mcRatio", mcRatio);
    if (essayRatio !== undefined) formData.append("essayRatio", essayRatio);
